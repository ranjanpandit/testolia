...
function StatusBadge({ status }) {
  const base = "inline-flex items-center px-3.5 py-1 rounded-full text-xs font-semibold border";
...
export default function TestList() {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("This assessment blueprint will be permanently deleted. Continue?")) {
      return;
    }
  ...
  // Client-side filtering (you can later move to server-side)
          ...
          <table className="w-full min-w-[900px] border-collapse">
            <thead>
              <tr className="bg-slate-50/70 border-b border-slate-100">
                {["Test", "Duration", "Status", "Schedule", "Actions"].map((header) => (
                  <th
                    key={header}
                    className={cn(
                ...
                <tr>
                  <td colSpan={5} className="py-24 text-center">
                    <p className="text-slate-400 font-medium">
                      {searchTerm
                        ? "No matching assessment blueprints found"
                    ...
                    <td className="px-6 py-5">
                      <div className="flex items-center justify-end gap-1 opacity-70 group-hover:opacity-100 transition-opacity">
                        ...
                        <ActionIconButton
                          ...
                          variant="destructive"
                          disabled={test.status === "published" || deletingId === test.id}
